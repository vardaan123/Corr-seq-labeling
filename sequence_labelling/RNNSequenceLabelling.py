...
__all__ = ["RNNSequenceLabelling"]

class RNNSequenceLabelling():
  """ RNN model for sequence labelling
  """
  ...
  def inference(self, seq_inputs, seq_lengths, 
    ...
    if self.dynamic_unrolling:
      outputs, states = tf.nn.dynamic_rnn(cell, inputs, 
        ...
        sequence_length=seq_lengths)    #outputs shape: [batch_size, max_seq_length, rnn_size]
    ...
    with tf.name_scope('softmax'):      
      ...
      outputs_flat = tf.reshape(outputs_drop, [-1, self.rnn_size])
