...
from deep.dataloaders.punct_data_loaders import load_single_multitask_dataset
...
test_data = load_single_multitask_dataset(test_seqfile,test_labfile,
...
for task_id in range(model.args.num_tasks):
  with open(os.path.join(model.args.save_dir,'%s_task%d.tsv' % (outfile_prefix,task_id)),'w') as f:
