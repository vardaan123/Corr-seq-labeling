...
def generate_metrics_task(goldlist,predlist,punct_list,task_id): # {PUNCT:[2,3],COMMA:[2],PERIOD:[3]}
    ...
    for punct in punct_list:
        ...
        print '%s,%s,%f,%f,%f,%f,%f' % (sys.argv[task_id+3],':'.join([str(x) for x in punct]),prec,recall,f1score,SER,ERR)

    prec_overall,recall_overall,f1score_overall = calculate_overall_metrics(goldlist,predlist,labellist)
    print '%s,%s,%f,%f,%f,%f,%f' % (sys.argv[task_id+3],'overall',prec_overall,recall_overall,f1score_overall,SER,ERR)
...
try:
    ...
    print 'asr_punct_slots_count = %d' % asr_punct_slots_count
...
for task_id in range(num_tasks):
    ...
    generate_metrics_task(gold_labels_list_filt,predlist_filt,punct_list,task_id)
