""" training script for CorrelatedSequenceLabelling
...
__all__ = ["TrainCorrelatedSequenceLabelling"]

def TrainCorrelatedSequenceLabelling(args,train_data,valid_data):
  """ training function
  """
  ...
  model = CorrelatedSequenceLabelling(
    ...
    model=args.model,
    use_hidden_layer=bool(args.use_hidden_layer),
    hidden_dimension=args.hidden_dimension,
    hidden_activation=args.hidden_activation,
    num_tasks=args.num_tasks,
    task_weights=[float(x) for x in args.task_loss_weight.split(':')],
  ...
  with tf.Graph().as_default():  
    ...
    labels = tf.placeholder(tf.int32, shape=(args.num_tasks,None, None), name="labels")
    ...
    logits_op = model.inference(seq_inputs, seq_lengths,  #returns a list
    ...
    accuracy_op = model.evaluation(logits_op, labels) #returns a list
    ...
    #valid_summary_op = tf.merge_summary([loss_summary])
    ...
    def evaluate_model(sess,
      ...
      """
      batch_accuracy = [0.0] * args.num_tasks 
      batch_loss = 0.0
      ...
      for step in xrange(steps_per_epoch):
        ...
        accuracy_list = sess.run(accuracy_op,feed_dict=feed_dict)
	batch_accuracy = [batch_accuracy[i]+accuracy_list[i]*np.sum(batch_seq_lengths) for i in range(len(accuracy_list))]
    ...
    with tf.Session(config=session_conf) as sess:
      ...
      while train_data.epochs_completed <= args.max_epochs:
        ...
        if step % args.print_every == 0:
          print('epoch %d batch %d: loss = %.3f accuracy = %s (%.3f secs)' % (train_data.epochs_completed+1,
            step,
            loss,
        ...
        if train_data.epochs_completed > previous_epoch:
          ...
          if train_data.epochs_completed % args.evaluate_every_epochs == 0 or train_data.epochs_completed == args.max_epochs-1:
            ...
            print('----------------------------------train accuracy : %s loss : %0.03f' %(':'.join([str(i) for i in accuracy]),loss))          
            performance['train_loss'].append(loss)
            performance['train_accuracy'].append(':'.join([str(i) for i in accuracy]))
            ...
            print('----------------------------------valid accuracy : %s loss : %0.03f' %(':'.join([str(i) for i in accuracy]),loss))
            performance['valid_loss'].append(loss)
            performance['valid_accuracy'].append(':'.join([str(i) for i in accuracy]))
...
if __name__ == '__main__':
  ...
  parser.add_argument('--use_hidden_layer', type=int, default=0, help='hidden layer on top of RNN output')
  parser.add_argument('--hidden_dimension', type=int, default=100, help='number of hidden units if use_hidden_layer is set to 1')
  parser.add_argument('--hidden_activation', type=str, default='linear', help='choose out of linear, sigmoid, tanh, relu.')
  parser.add_argument('--task_loss_weight', type=str, default=None, help='loss weights concatenated by colon, for ex, two tasks could be 0.6:0.4 or 0.5:0.5, the sum should be 1.')

  
  parser.add_argument('--dynamic_unrolling', type=int, default=1, help='if 1 do dynamic calculations when unrolling the RNN') #unused
  parser.add_argument('--use_cost_sensitive_loss', type=int, default=0, help='if 1 use_cost_sensitive_loss only if binary classification') #unused
  ...
  train_data, valid_data, test_data = load_multitask_dataset(args.trainFilename,
    args.devFilename,
    args.testFilename,
  ...
  if args.task_loss_weight==None:
        task_weights = [1.0/args.num_tasks] * args.num_tasks
  else:
        templist = args.task_loss_weight.split(':')
        task_weights = [float(x) for x in templist]
        if sum(task_weights) != 1.0 or len(task_weights) != args.num_tasks:
            task_weights = [1.0/args.num_tasks] * args.num_tasks
  print task_weights
  args.task_loss_weight = ':'.join([str(x) for x in task_weights])
            
  # ==================================================
  # TRAINING
  # ==================================================

  TrainCorrelatedSequenceLabelling(args,train_data,valid_data)
